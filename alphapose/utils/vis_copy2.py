...
def vis_frame(frame, im_res, opt, format='coco'):
    ...
    if kp_num == 17:
        if format == 'coco':
            ...
            """
            # wz 全部修改为CYAN
            p_color = [CYAN for i in range(18)]
            line_color = [ CYAN for i in range(15)]
    ...
    for human in im_res['result']:
        ...
        if kp_num == 17:
            ...
            kp_scores = torch.cat((kp_scores, torch.unsqueeze((kp_scores[5, :] + kp_scores[6, :]) / 2, 0)))
        if opt.tracking:
            color = ORANGE # get_color_fast(int(abs(human['idx'])))   # wz 如果添加了跟踪选项
        else:
            color = BLUE
        ...
        for i in range(5):
            if kp_preds[i][0] == xmin:
                ymin = kp_preds[i][1] - radius
            if kp_preds[i][0] == xmax:
                ymax = kp_preds[i][1] + radius
        ...
        if opt.showbox:
            ...
            #head_bbox
            cv2.rectangle(img,(int(abs(head_bbox[0])), int(abs(head_bbox[1]))), (int(abs(head_bbox[2])), int(abs(head_bbox[3]))), color, 1)
            ...
            SCALE = 0.25
            #(xmin, ymin) --> right
            cv2.line(img, (int(bbox[0]), int(bbox[2])), (int(bbox[0] + SCALE * (bbox[1] - bbox[0])), int(bbox[2])), LIGHT_BLUE, 5)
            #(xmin, ymin) --> down
            cv2.line(img, (int(bbox[0]), int(bbox[2])), (int(bbox[0]), int(bbox[2] + SCALE * (bbox[3] - bbox[2]))), LIGHT_BLUE, 5)
            #(xmax, ymin) --> left
            cv2.line(img, (int(bbox[1]), int(bbox[2])), (int(bbox[1] - SCALE * (bbox[1] - bbox[0])), int(bbox[2])), LIGHT_BLUE, 5)
            # (xmax. xmin) --> down
            cv2.line(img, (int(bbox[1]), int(bbox[2])), (int(bbox[1]), int(bbox[2] + SCALE * (bbox[3] - bbox[2]))), LIGHT_BLUE, 5)
            # (xmin, ymax) --> right
            cv2.line(img, (int(bbox[0]), int(bbox[3])), (int(bbox[0] + SCALE * (bbox[1] - bbox[0])), int(bbox[3])), LIGHT_BLUE, 5)
            # (xmin, ymax) --> up
            cv2.line(img, (int(bbox[0]), int(bbox[3])), (int(bbox[0]), int(bbox[3] -SCALE * (bbox[3] - bbox[2])),), LIGHT_BLUE, 5)
            # (xmax  ymax) --> left
            cv2.line(img, (int(bbox[1]), int(bbox[3])), (int(bbox[1] - SCALE * (bbox[1] - bbox[0])), int(bbox[3])), LIGHT_BLUE, 5)
            # (xmax  ymax) --> up
            cv2.line(img, (int(bbox[1]), int(bbox[3])), (int(bbox[1]), int(bbox[3] - SCALE * (bbox[3] - bbox[2]))), LIGHT_BLUE, 5)
        ...
        for n in range(kp_scores.shape[0]):
            ...
            if n < len(p_color):
                if opt.tracking:
                    ...
                    # 画同心圆
                    cv2.circle(bg, (int(cor_x), int(cor_y)), 6, ORANGE, -1)
                else:
                    cv2.circle(bg, (int(cor_x), int(cor_y)), 4, p_color[n], -1)
                    # 画同心圆
                    cv2.circle(bg, (int(cor_x), int(cor_y)), 6, ORANGE, -1)
