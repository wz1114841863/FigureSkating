...
from server.skating2.redis_base.cache_redis import wirte_into_queue, redis_pool
import redis
...
parser.add_argument('--detbatch', type=int, default=15,
    ...
    if True:
        for i in im_names_desc:
            ...
            with torch.no_grad():
                ...
                # hm = hm.cpu()
                writer.save(boxes, scores, ids, hm.cpu(), cropped_boxes, orig_img, im_name)
                # # save hm to video
                # assert hm.size(0) == 1, "should be only one person"
                # hm = hm * 100
                # import pdb;pdb.set_trace()
                hm_bg = np.zeros((frame_height, frame_width))
                if len(hm.shape) < 4:
                    ...
                    # pdb.set_trace()
                    hm_pos = [hm_ul[0], hm_ul[0] + hm_height, hm_ul[1], hm_ul[1] + hm_width]
                    hm = F.interpolate(hm, size=(hm_height, hm_width), mode='bilinear', align_corners=True)
                    hm = hm.sum(0).sum(0).cpu().numpy()
                    hm_crop = np.zeros(4, dtype=int)
                    ...
                    if hm_pos[1] > frame_height - 1:
                        hm_crop[1] = hm_pos[1] - frame_height + 1
                        hm_pos[1] = frame_height - 1
                        
                    if hm_pos[3] > frame_width - 1:
                        hm_crop[3] = hm_pos[3] - frame_width + 1
                        hm_pos[3] = frame_width - 1
                    # pdb.set_trace()
                    hm_bg[hm_pos[0]:hm_pos[1], hm_pos[2]:hm_pos[3]] = hm[hm_crop[0]:hm_height-hm_crop[1], hm_crop[2]:hm_width-hm_crop[3]]
                hm = hm_bg
                ...
                if ensemble == True:
                    output = cv.resize(output,(340,250))
                    ret, jpeg = cv.imencode('.jpg', output)
        ...
        keypoints = np.array(writer.input_3d)[np.newaxis, :, :, :]
        print("keypoints.shape: ", keypoints.shape)
        np.savez_compressed('/home/dell/wz/AlphaPose/examples/output/input_3d.npz', reconstruction=keypoints)
